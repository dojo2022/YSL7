...
//		}
	...
	//uDelete
		public int uDelete() throws UnsupportedEncodingException  {
			request.setCharacterEncoding("UTF-8");
...
//				request.setAttribute("three2", three2);
		...
		//コンボボックスの値を取得してくるサービスを呼ぶ
		BUserService service = new BUserService();
		ArrayList<ArrayList<RoleBeans>> allRole = service.getRole();
		System.out.println(allRole.size()+"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
		HttpSession session = request.getSession();
		session.setAttribute("role1",allRole.get(0));
		session.setAttribute("role2",allRole.get(1));
		session.setAttribute("role3",allRole.get(2));
