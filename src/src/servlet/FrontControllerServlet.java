...
public class FrontControllerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("page_id")!=null && request.getParameter("bt_name")!=null) {
			...
			} else if (pageId.equals("FH01") && button.equals("message")) {//home.jspからmessage.jspへ
				path = "WEB-INF/jsp/message.jsp";
			} else if (pageId.equals("FH01") && button.equals("regist_feeling")) {//hpme.jspからregist_feeling.jsp
				path = "WEB-INF/jsp/regist_feeling.jsp";
			} else if (pageId.equals("FH01") && button.equals("history_feeling")) {//home.jspからhistory_feeling.jsp
				HistoryFeelingAction action = new HistoryFeelingAction(request);
				path = action.showHistory;
			} else if (pageId.equals("FH01") && button.equals("graph_feeling")) {//home.jspからgraph_feeling.jsp
				GraphFeelingAction action = new GraphFeelingAction(request);
				path = action.showGraph();
			} else if ((pageId.equals("FH01") && button.equals("evaluation")) || (pageId.equals("FE02") && button.equals("evaluation"))) {//home.jspからsearch_evaluation.jsp
				SearchEvaluationAction action = new SearchEvaluationAction(request);
				path = action.showEmp();
			} else if (pageId.equals("FH01") && button.equals("hr")) {//home.jspかlist.jsp(人事部画面へ)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pageId.equals("FM01") && button.equals("送信")) {
			MessageAction action = new MessgaeAction(request);
